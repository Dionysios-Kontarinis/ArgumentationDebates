 ...
 * Each one of the strategies 1, 2A, 2B, 3A, 3B, 3C, 4A, 4B, 0 (the random strategy) is put up against every other one (81 strategy profiles in total).
 ...
 *
 */
public class MainDebates_9x9profiles_WithRandom_AllInfo {
...
//			merged.printGB(null);
			...
			for (int j=0; j<numOfDebates; j++) {
				...
				else if (stratPROindicator == 8) strategyPRO = "0";	
				...
				else if (stratCONindicator == 8) strategyCON = "0";
...
//					if (j == 0) move = currAg.strategyChangeIssue(gb);
					if (currAg.team.equals("PRO")) {
						...
						else if (strategyPRO.equals("0")) move = currAg.strategyRandom(gb);
					} else {
						...
						else if (strategyCON.equals("0")) move = currAg.strategyRandom(gb);
					}
...
//		}
		...
		for (int i=0; i<numOfConfigs; i++) {
			...
			writer_results.print("StrategyPRO\\StrategyCON" + " " + "1" + " " + "2A" + " " + "2B" + " " + "3A" + " " + "3B" + " " + "3C" + " " + "4A" + " " + "4B" + " " + "0");
			writer_results.println();
			for (int j=0; j<9; j++) {
				...
				else if (j == 8) writer_results.print("0" + " ");
...
//				else if (w == 8) writer_results.print("0" + " ");
//				int totalPROwins = 0;
//				for (int x=0; x<9; x++) {
...
//		}
		...
		int line0vs0 = 8, column0vs0 = 8;
		int agree_1vs1_2vs2 = 0;
		int agree_1vs1_3Cvs3C = 0;
		int agree_3Cvs3C_0vs0 = 0;
		for (int i=0; i<numOfConfigs; i++) {
			for (int j=0; j<numRepetitions; j++) {
				...
				if (issueInDebate[i][90*line3Cvs3C + 10*column3Cvs3C + j] == issueInDebate[i][90*line0vs0 + 10*column0vs0 + j]) {
					agree_3Cvs3C_0vs0++;
				}
			}
		}
		...
		writer_results.println("AGREEMENT_3Cvs3C_WITH_0vs0 =" + (float)agree_3Cvs3C_0vs0/(numOfConfigs*numRepetitions));
